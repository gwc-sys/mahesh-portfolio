...
const terminalCommands = ['help', 'about', 'skills', 'projects', 'experience', 'certifications', 'github', 'contact', 'resume', 'open <app>', 'date', 'whoami', 'history', 'clear'];
function TerminalApp({ open }: { open: (id: AppId) => void }) {
  const [lines, setLines] = useState<string[]>(['Mahesh Portfolio Terminal', 'Type "help" to see available commands.']); const [input, setInput] = useState(''); const [history, setHistory] = useState<string[]>([]); const [historyIndex, setHistoryIndex] = useState(-1); const end = useRef<HTMLDivElement>(null);
  useEffect(() => end.current?.scrollIntoView(), [lines]);
  const run = (raw: string) => {
    const normalized = raw.trim(); if (!normalized) return;
    const [command, ...args] = normalized.toLowerCase().split(/\s+/); const prompt = `mahesh@portfolio:~$ ${normalized}`;
    const githubUrl = socialLinks.find((link) => link.label === 'GitHub')?.href ?? '';
    const output: Record<string, string[]> = {
      help: ['Available commands:', ...terminalCommands.map((item) => `  ${item}`), 'Tip: use ↑ and ↓ to browse command history.'],
    ...
    else if (command === 'open') { const target = args[0] === 'certificates' ? 'certifications' : args[0]; if ((apps.map((app) => app.id) as string[]).includes(target)) { open(target as AppId); setLines((current) => [...current, prompt, `Opening ${target}...`]); } else setLines((current) => [...current, prompt, `open: ${target || 'missing app name'}: application not found`]); }
  ...
  const submit = (e: FormEvent) => { e.preventDefault(); run(input); setInput(''); }; const keys = (e: KeyboardEvent<HTMLInputElement>) => { if (e.key === 'ArrowUp' && history.length) { e.preventDefault(); const i = Math.min(historyIndex + 1, history.length - 1); setHistoryIndex(i); setInput(history[history.length - 1 - i]); } if (e.key === 'ArrowDown') { e.preventDefault(); const i = historyIndex - 1; setHistoryIndex(i); setInput(i >= 0 ? history[history.length - 1 - i] : ''); } };
  return <div className="terminal-app"><div className="terminal-output">{lines.map((line, i) => <TerminalLine key={`${line}-${i}`} line={line} />)}<div ref={end} /></div><div className="terminal-quick">{['help', 'about', 'projects', 'skills', 'contact', 'clear'].map((command) => <button key={command} onClick={() => run(command)}>{command}</button>)}</div><form onSubmit={submit}><span>mahesh@portfolio:~$</span><input autoFocus value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={keys} aria-label="Terminal command" autoComplete="off" spellCheck={false} /></form></div>;
}
...
function ResumeApp({ open }: { open: (id: AppId) => void }) { return <div className="resume-app"><div className="resume-sheet"><span>MR</span><p>MAHESH RASKAR</p><h2>Full-Stack Developer</h2><hr /><h3>Profile</h3><p>{personalInfo.about}</p><h3>Core toolkit</h3><p>{personalInfo.skillsSummary}</p><h3>Experience</h3><p>{personalInfo.experienceSummary}</p></div><aside><span className="eyebrow">RESUME</span><h2>Mahesh Raskar</h2><p>Preview the resume, open it in a new tab, or download a local copy.</p><a className="primary-button" href={personalInfo.resumeUrl} download><TbDownload /> Download Resume</a><a href={personalInfo.resumeUrl} target="_blank" rel="noreferrer"><TbExternalLink /> Open Resume</a><button onClick={() => open('contact')}><TbMail /> Contact Me</button></aside></div>; }
